...
class ChatPIE:
    ...
    def get_cost(self):
        ...
        prompt_costs['gpt-4o'] = 0.00250
        response_costs['gpt-4o'] = 0.01000
...
class ChatAPI:
    ...
    def get_response(self, model, message_history : MessageHistory, json_format = False):
        ...
        if model not in self.total_tokens_used:
            self.total_tokens_used[model] =0
        if not json_format:
            completion = self.client.chat.completions.create(model=model,
                                                             messages=message_history.get_messages(),
            ...
            completion = self.client.chat.completions.create(model=model,
                    messages=message_history.get_messages(), response_format={'type':'json_object'}, temperature=self.temperature)
        ...
        #self.total_tokens_used[model] += completion.usage.prompt_tokens_details.total_tokens
