...
class LLM4PP_Driver:
    ...
    def save_one_response_jsonl(self, path: str, data: List[dict], append: bool = False):
    ...
    def evaluate(self):
        ...
        class statistics:
            ...
            logsum : float
            total_compiled : int
            total_greater_speedup: int




        category_statistics = dict()
        all_statistics = statistics(addone=False, total=0, total_correct=0, summation=0, logsum=0, total_compiled=0, total_greater_speedup=0)
        ...
        for x in self.responses:
            ...
            if not x.correct:
                ...
                speedup = x.reference_runtime / x.runtime
            ...
            if speedup >= 2:
                category_statistics[problem_cat].total_greater_speedup += 1
                all_statistics.total_greater_speedup += 1

            category_statistics[problem_cat].logsum += logratio
            category_statistics[problem_cat].summation += speedup
        ...
        for category in category_statistics.keys():
            table.add_row([category,\
                       ...
                       category_statistics[category].summation / category_statistics[category].total,\
                       category_statistics[category].total_greater_speedup / category_statistics[category].total])
        table.add_row(["all",\
                   ...
                   all_statistics.summation / all_statistics.total,\
                   all_statistics.total_greater_speedup / all_statistics.total])
        print(table)
