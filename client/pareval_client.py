...
def pareval_submit(problem_category: str, problem_unique_id: str, optimized_code: str, header: str, mode: str):
...
class ParEvalProblemLoader(ProblemLoader):
    def __init__(self, mode: str, data_path : str):
...
class ParEvalSubmissionRunner(SubmissionRunner):
    ...
    def submit(self, submission : LLM4PP_Submission):
        ...
        result = pareval_submit(problem.category, problem.problem_id, submission.submitted_code, problem.header, problem.mode)
...
class ParEvalDriver(LLM4PP_Driver):
    def __init__(self, mode, data_path="ParEval-PolyBench-Code-Opt/prompts/pareval_code_opt.json"):
        if mode == 'serial':
            self.mode = mode
            ...
            self.mode = 'serial'
        self.problem_loader = ParEvalProblemLoader(self.mode, data_path)
