...
def create_generation_dict(problem_category: str, problem_unique_id: str, optimized_code: str, mode: str, prompt: str = ""):
    d = {}
    d["outputs"] = []
    ...
    d["parallelism_model"] = mode # the mode is either serial or omp
    d["language"] = "cpp"
    d["prompt"] = prompt
...
def run_driver(gen_file: str, problem_sizes_file: str = "problem-sizes.json", run_timeout: int = 300, launch_configs: str = "launch-configs-speedcode.json") -> str:
    ...
    with tempfile.NamedTemporaryFile(suffix=".json") as output_f:
        ...
        proc = subprocess.run(subprocess_args, cwd="ParEval-PolyBench-Code-Opt/drivers", capture_output=True, text=True)
...
def polybench_submit(problem_category: str, problem_unique_id: str, optimized_code: str, header: str, mode: str):
    gen_dict = create_generation_dict(problem_category, problem_unique_id, optimized_code, mode, prompt=header)
...
class PolyBenchProblemLoader(ProblemLoader):
    def __init__(self, mode, data_path : str):
        ...
        if mode == 'serial':
            ...
            print(f"Unknown {mode}, set number of cpus to 1.")
        with open(data_path) as f:
            lst_problems = json.load(f)
            
            for problem_dict in lst_problems:
                # for i in range(100):
                llm4pp_problem = LLM4PP_Problem(problem_id=problem_dict["name"], source_code=problem_dict["src_code"], category=problem_dict["problem_type"], header=problem_dict["prompt"], mode=mode, target_benchmark = BenchmarkDescription(num_cpus=num_cpus))
                self.problem_list.append(llm4pp_problem)

        self.problem_iterator = ProblemIterator(self.problem_list)

class PolyBenchSubmissionRunner(SubmissionRunner):
    def __init__(self):
        pass
    ...
    def submit(self, submission : LLM4PP_Submission):
        ...
        result = polybench_submit(problem.category, problem.problem_id, submission.submitted_code, problem.header, problem.mode)
...
class PolyBenchDriver(LLM4PP_Driver):
    def __init__(self, mode, data_path="ParEval-PolyBench-Code-Opt/prompts/polybench_code_opt.json"):
        if mode == 'serial':
            self.mode = mode
        elif mode == 'OpenMP' or mode == 'openmp': 
            self.mode = 'omp'
        else:
            print(f"Unknown mode {mode}, using serial model instead")
            self.mode = 'serial'
        self.problem_loader = PolyBenchProblemLoader(self.mode, data_path)
        self.runner = PolyBenchSubmissionRunner()
        self.problems = []
        self.responses = []
